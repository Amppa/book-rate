...
class TestGooglePlayScraper(unittest.TestCase):
    ...
    def test_extract_volume_id(self):
        ...
        # Case 1: from query starting with gb:
        rating1 = SourceRating(source_name="Google Books", query="gb:z2z_6hLoPmgC")
        self.assertEqual(source._extract_volume_id(rating1), "z2z_6hLoPmgC")
        
        # Case 2: from url
        rating2 = SourceRating(source_name="Google Books", url="https://play.google.com/store/books/details?id=ZuKTvERuPG8C")
        self.assertEqual(source._extract_volume_id(rating2), "ZuKTvERuPG8C")
        
        # None rating
        self.assertIsNone(source._extract_volume_id(None))

    @patch("book_rate.sources.google_books.GoogleBooksSource._fetch_html")
    def test_fetch_google_play_rating_json_ld(self, mock_fetch_html):
        ...
        rate, count = source._fetch_google_play_rating("test_id")
    ...
    @patch("book_rate.sources.google_books.GoogleBooksSource._fetch_google_play_rating")
    @patch("book_rate.sources.google_books.GoogleBooksSource._fetch_ratings")
    def test_fetch_ratings_enrichment(self, mock_fetch_ratings, mock_fetch_play):
        source = GoogleBooksSource()
        
        # Setup mock for _fetch_ratings
        mock_rating = SourceRating(
            source_name="Google Books",
            rate=4.0,
            rating_count=35,
            url="https://books.google.com/books?id=ZuKTvERuPG8C"
        )
        mock_rating.status = "MATCH"
        mock_fetch_ratings.return_value = mock_rating
        
        # Setup mock for Play rating
        mock_fetch_play.return_value = (4.487, 630)
        ...
        mock_fetch_play.assert_called_once_with("ZuKTvERuPG8C")
